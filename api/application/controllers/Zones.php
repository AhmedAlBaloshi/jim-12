...
class Zones extends CI_Controller{
    ...
    public $_table_column_array = ['name_en','name_ar','coordinates','type','city','shipping_priority','shipping_price','shipping','status', 'drivers_shipping_price'];
    public $_table_column_edit = ['id','name_en','name_ar','coordinates','type','city','shipping_priority','shipping_price','shipping','status', 'drivers_shipping_price'];
    public $required = ['id'];
    ...
    public function getById(){
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Zones_model->getById($_POST['id']);
                if($result != null){
    ...
    public function editZone(){
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Zones_model->editList($_POST,$_POST['id']);
    ...
    public function createZone(){
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Zones_model->saveList($_POST);
                if($result != null){
                    $id = $this->db->insert_id();
                    $data = $this->Zones_model->getByIdValue($id);
                    echo $this->json->response($data,$this->_OKmessage,$this->_statusOK);
                }else{
                    echo $this->json->response(['error'=>'Something Went Wrong.'],$this->_Errmessage,$this->_statusErr);
                }
    ...
    public function deleteList(){
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Zones_model->deleteList($_POST['id']);
                if($result != null){
                    echo $this->json->response($result,$this->_OKmessage,$this->_statusOK);
                }else{
                    echo $this->json->response(['error'=>'Something Went Wrong.'],$this->_Errmessage,$this->_statusErr);
